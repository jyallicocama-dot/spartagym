...
const Pagos = () => {
  ...
  const totalPagosMensuales = pagos.filter(p => p.tipo === 'mensual').reduce((acc, p) => acc + p.monto, 0)
  const totalPagosDiarios = pagos.filter(p => p.tipo === 'diario').reduce((acc, p) => acc + p.monto, 0)
  const totalPagosTrimestrales = pagos.filter(p => p.tipo === 'trimestral').reduce((acc, p) => acc + p.monto, 0)
  ...
  const handleSubmit = async (e) => {
    ...
    if (resultado) {
      toast.success('¡Pago Registrado!', `Pago ${formData.tipo} de S/${formData.monto} para ${cliente?.nombre || 'cliente'}`)
    } else {
        ...
        >
          <div>
            <h1 className="font-sparta text-3xl font-bold mb-2">
              <span className="text-gradient">REGISTRO DE</span>
              <span className="text-white"> PAGOS</span>
            </h1>
            <p className="text-gray-400">Gestiona los pagos de membresías</p>
          </div>
          ...
          <div className="card-sparta p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-white">S/ {(totalPagosMensuales + totalPagosDiarios).toFixed(2)}</p>
              </div>
            </div>
          </div>
          <div className="card-sparta p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-gray-400">Pagos Mensuales</p>
                <p className="text-xl font-bold text-white">S/ {totalPagosMensuales.toFixed(2)}</p>
              </div>
            </div>
          </div>
          <div className="card-sparta p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-gray-400">Pagos Diarios</p>
                <p className="text-xl font-bold text-white">S/ {totalPagosDiarios.toFixed(2)}</p>
              </div>
            </div>
          </div>
          <div className="card-sparta p-4">
                  ...
                  <tr key={pago.id}>
                    <td>
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-medium text-white">{pago.clienteNombre}</span>
                      </div>
                    </td>
                    <td>
                    ...
                    <td className="text-gray-300">{pago.mes || 'Día único'}</td>
                    <td className="text-gray-300">{pago.fecha}</td>
                    <td className="text-sparta-gold font-bold">S/ {pago.monto.toFixed(2)}</td>
                  </tr>
                )) : (
                  <tr>
              ...
              >
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {(formData.tipo === 'mensual' || formData.tipo === 'trimestral') && (
                    <div>
                      <label className="block text-sm text-gray-400 mb-2">Mes a Pagar</label>
                      <select
                        value={formData.mes}
